...
data Find {A : Set}(p : A → Bool) : List A → Set where
  found : (xs : List A)(y : A) → isTrue (p y) → (ys : List A) → Find p (xs ++ y ∷ ys)
  not-found : ∀ {xs} → All (isFalse ∘ p) xs → Find p xs
find : {A : Set} (p : A → Bool) → (xs : List A) → Find p xs
find p [] = not-found ∅
find p (x ∷ xs) with inspect (p x)
find p (x ∷ xs) | true with-≡ px≡true = found [] x (lemmaT px≡true) xs
find p (x ∷ xs) | false with-≡ px≡false with find p xs
find p (x ∷ .(xs ++ y ∷ ys)) | false with-≡ px≡false | found xs y prf ys = found (x ∷ xs) y prf ys
find p (x ∷ xs) | false with-≡ px≡false | not-found npxs = not-found ((lemmaF px≡false) :all: npxs)
